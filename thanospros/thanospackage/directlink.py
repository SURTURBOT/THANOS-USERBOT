...
async def direct_link_generator(request):
    ...
    THANOSBOTevent = await edit_or_reply(request, "`Processing...`")
    ...
    if message:
        ...
        await THANOSBOTevent.edit("`Usage: .direct <url>`")
        return
    reply = ""
    links = re.findall(r"\bhttps?://.*\.\S+", message)
    if not links:
        reply = "`No links found!`"
        await THANOSBOTevent.edit(reply)
    for link in links:
...
def yandex_disk(url: str) -> str:
    ...
    try:
        ...
        return reply
    api = "https://cloud-api.yandex.net/v1/disk/public/thanospackage.thanosresource/download?public_key={}"
    try:
