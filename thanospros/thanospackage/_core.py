...
async def send(event):
    ...
    the_plugin_file = "./thanospros/thanospackage/{}.py".format(input_str)
    the_1plugin_file = "./thanospros/thanospackage/Abuse/{}.py".format(input_str)
    the_2plugin_file = "./thanospros/thanospackage/Spam/{}.py".format(input_str)
    the_3plugin_file = "./thanospros/thanospackage/xtra_package/{}.py".format(input_str)
        ...
        try:
            downloaded_file_name = (
                await event.client.download_media(  # pylint:disable=E0602
                    ...
                    "./thanospros/thanospackage/",  # pylint:disable=E0602
                )
            )
...
async def uninstall(event):
    ...
    dir_path = f"./thanospros/thanospackage/{shortname}.py"
    try:
...
async def install(event):
    ...
    cmd = "ls thanospros/thanospackage"
    thumb = core_pic
    ...
    if len(OUTPUT) > 69:
        with io.BytesIO(str.encode(OUTPUT)) as out_file:
            ...
            await edit_or_reply(
                THANOSBOT_file,
                f"**Output Too Large. This is the file for the list of thanospackage in THANOS-PRO**.\n\nBY :- **{DEFAULTUSER}**",
            )
            await event.delete()
...
CmdHelp("core").add_command(
    "install",
    "<reply to a .py file>",
    "Installs the replied python file if suitable to thanospros codes. (TEMPORARILY DISABLED AS HACKERS MAKE YOU INSTALL SOME PLUGINS AND GET YOUR DATA)",
).add_command(
    "uninstall",
    "<plugin name>",
    "Uninstalls the given plugin from thanospros. To get that again do .restart",
    "uninstall alive",
).add_command(
    "load", "<plugin name>", "Loades the unloaded plugin to your thanospros", "load alive"
).add_command(
    "unload", "<plugin name>", "Unloads the plugin from your thanospros", "unload alive"
).add_command(
    "send",
    "<file name>",
    "Sends the given file from your thanospros server, if any.",
    "send alive",
).add_command(
