...
async def _(event):
    ...
    if event.reply_to_msg_id:
        ...
        event = await edit_or_reply(event, "Adding rishabhai to user")
        que[e] = []
        qeue = que.get(e)
        appendable = [e]
        qeue.append(appendable)
        await event.edit(f"Added rishabhai To User")
    else:
...
async def _(event):
    ...
    if event.reply_to_msg_id:
        ...
        c = b.first_name
        username = f"[{c}](tg://user?id={e})"
        event = await edit_or_reply(event, "Removin rishabhai From user")
        queue = que.get(e)
        queue.pop(0)
        await event.edit(f"Removed successfully")
    else:
        user = event.pattern_match.group(1)
        event = await edit_or_reply(event, "Removing rishabhai From User")
        a = await event.client.get_entity(user)
        e = a.id
        c = a.first_name
        username = f"[{c}](tg://user?id={e})"
