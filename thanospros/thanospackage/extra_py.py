...
from thanospros.utils import admin_cmd, load_module
from var import Var
...
async def install(event):
    ...
    for ixo in total_doxx:
        ...
        downloaded_file_name = await event.client.download_media(
            await borg.get_messages(chat, ids=mxo), "thanospros/thanospackage/"
        )
...
CmdHelp("extra_py").add_command(
    "extdl",
    None,
    "Installs all thanospackage from the channal which id is in PLUGIN_CHANNEL variable",
).add_command(
    "installall",
    "<channel/grp username>",
    "Installs all the thanospackage in provided channel / group. (May get floodwait error)",
).add()
