...
from thanospros import bot as borg
...
async def gben(thanospros):
    dc = dark = thanospros
    i = 0
    await dc.get_sender()
    me = await thanospros.client.get_me()
    ...
    if thanospros.is_private:
        ...
        rank = thanospros.pattern_match.group(1)
    else:
        thanospros.chat.title
    try:
        user, rank = await get_full_user(thanospros)
    except:
        pass
    if me == user:
    ...
    if user:
        ...
        for x in telchanel:
            try:
                await thanospros.client(EditAdminRequest(x, user, rgt, rank))
                i += 1
